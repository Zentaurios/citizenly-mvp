-- Citizenly Database Schema - Supabase Optimized
-- Version: 1.1 (Supabase Integration)
-- Description: Core schema for civic engagement platform with Supabase RLS
...
CREATE TABLE users (
    ...
    supabase_user_id UUID, -- Link to Supabase auth.users
...
-- Sessions table (for custom auth, not needed if using Supabase auth exclusively)
...
BEGIN
    ...
    IF auth.uid() IS NOT NULL THEN
        RETURN (SELECT id FROM users WHERE supabase_user_id = auth.uid());
    END IF;
    
    -- Fallback to custom session (for hybrid auth)
    RETURN COALESCE(
...
-- Users RLS Policies
CREATE POLICY "Users can view own profile" ON users FOR ALL
USING (id = get_current_user_id());

CREATE POLICY "Users can view verified politicians" ON users FOR SELECT
USING (role = 'politician' AND verification_status = 'verified' AND is_active = true);
...
CREATE POLICY "Service role full access" ON addresses FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');

-- Politicians RLS Policies
CREATE POLICY "Politicians can manage own profile" ON politicians FOR ALL
USING (user_id = get_current_user_id());
...
USING (
    EXISTS (
        ...
        WHERE u.id = politicians.user_id 
        AND u.verification_status = 'verified' 
        AND u.is_active = true
    )
);

CREATE POLICY "Service role full access" ON politicians FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');

-- Verification Attempts RLS Policies
CREATE POLICY "Users can view own verification attempts" ON verification_attempts FOR SELECT
USING (user_id = get_current_user_id());

CREATE POLICY "Users can create own verification attempts" ON verification_attempts FOR INSERT
WITH CHECK (user_id = get_current_user_id());

CREATE POLICY "Service role full access" ON verification_attempts FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');

-- Sessions RLS Policies (if using custom auth)
CREATE POLICY "Users can manage own sessions" ON sessions FOR ALL
USING (user_id = get_current_user_id());

CREATE POLICY "Service role full access" ON sessions FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');
...
CREATE POLICY "Verified users can view active polls" ON polls FOR SELECT
USING (
    is_active = true AND 
    EXISTS (
        SELECT 1 FROM users u 
...
CREATE POLICY "Service role full access" ON polls FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');

-- Poll Responses RLS Policies
CREATE POLICY "Users can manage own poll responses" ON poll_responses FOR ALL
USING (user_id = get_current_user_id());

CREATE POLICY "Politicians can view responses to their polls" ON poll_responses FOR SELECT
USING (
    poll_id IN (
        SELECT p.id FROM polls p
        JOIN politicians pol ON p.politician_id = pol.id
        WHERE pol.user_id = get_current_user_id()
    )
);

CREATE POLICY "Service role full access" ON poll_responses FOR ALL
USING (auth.jwt() ->> 'role' = 'service_role');
...
-- Insert initial data (only for development/testing)
...
COMMENT ON TABLE users IS 'Core user accounts with Supabase auth integration';
COMMENT ON COLUMN users.supabase_user_id IS 'Links to Supabase auth.users table';
COMMENT ON FUNCTION get_current_user_id() IS 'Helper function for RLS that works with both Supabase auth and custom auth';
